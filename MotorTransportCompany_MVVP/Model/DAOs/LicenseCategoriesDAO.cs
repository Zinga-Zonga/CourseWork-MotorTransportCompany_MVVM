    ...
    internal class LicenseCategoriesDAO : IDAO<LicenseCategory>
    {
        ...
        public LicenseCategory getEntityById(int id)
        {
            try
            {
                MySqlConnection connaction = new MySqlConnection(connectionString);
                connaction.Open();
                MySqlCommand command = new MySqlCommand($"SELECT id_category, fuel FROM `license_categories` WHERE id_category = {id}", connaction);
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    LicenseCategory entity = new LicenseCategory
                    {
        ...
        public void Add(LicenseCategory entity)
        {
            try
            {
                MySqlConnection connaction = new MySqlConnection(connectionString);
                connaction.Open();
                MySqlCommand command = new MySqlCommand($"INSERT INTO `license_categories` (`id_category`, `fuel`)" +
        ...
        public void Update(LicenseCategory entity)
        {
            try
            {
                ...
                MySqlConnection connaction = new MySqlConnection(connectionString);
                connaction.Open();
                MySqlCommand command = new MySqlCommand($"UPDATE license_categories SET id_category = '{entity.Id}', name = '{entity.Category}' WHERE license_categories.id_category = '{entity.Id}'", connaction);
                command.ExecuteNonQuery();
                connaction.Close();
            }
            catch (MySqlException ex)
